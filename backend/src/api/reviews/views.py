...
class ReviewsList(Resource):
    ...
    def get(self):
        """Return query result on reviews based on restaurant/user id"""
        parser = reqparse.RequestParser()
        parser.add_argument("user", type=int, required=False)
        parser.add_argument("restaurant", type=int, required=False)
        args = parser.parse_args()
        user_id = args.get("user")
        restaurant_id = args.get("restaurant")
        if user_id is None and restaurant_id is None:
    ...
    def post(self):
        ...
        response_object = {}
        if None in [user_id, restaurant_id, rating]:
            ...
            add_review(user_id, restaurant_id, rating)
...
class Reviews(Resource):
    ...
    def put(self, review_id):
        """Updates the star rating of a review."""
        post_data = request.get_json()
        rating = post_data.get("rating")
        response_object = {}
